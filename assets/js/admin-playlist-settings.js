(function () {
	...
	PillPicker.prototype._loadSaved = function () {
		...
		var items = [];
		if (name === 'jr_pinned_playlist_ids') {
			items = cfg.pinnedPlaylists || [];
		}
		if (name === 'jr_sidebar_playlist_id') {
			items = cfg.sidebarPlaylist || [];
		}
		if (name === 'jr_sidebar_video_ids') {
			items = cfg.sidebarVideos || [];
		}
		var self = this;
		items.forEach(
			function (item) {
	...
	PillPicker.prototype._fetch = function (q) {
		...
		.then(
			function (data) {
				if (data.success) {
					self._renderDropdown( data.data );
				}
			}
		)
		.catch( function () {} );
	...
	PillPicker.prototype._renderDropdown = function (items) {
		var self        = this;
		var selectedIds = this.selected.map(
			function (s) {
				return s.id; }
		);
		var visible     = items.filter(
			function (item) {
	...
	PillPicker.prototype._syncHidden = function () {
		this.hidden.value = this.selected.map(
			function (s) {
				return s.id; }
		).join( ',' );
	};
	...
	function initSourceToggle() {
		...
		function update() {
			...
			document.querySelectorAll( '.jr-source-row--count' ).forEach(
				function (row) {
					row.style.display = val !== 'videos' ? '' : 'none';
				}
			);
			document.querySelectorAll( '.jr-source-row--playlist' ).forEach(
				function (row) {
					row.style.display = val === 'playlist' ? '' : 'none';
				}
			);
			document.querySelectorAll( '.jr-source-row--videos' ).forEach(
				function (row) {
					row.style.display = val === 'videos' ? '' : 'none';
				}
			);
	...
	document.addEventListener(
		'DOMContentLoaded',
		function () {
			document.querySelectorAll( '.jr-pill-picker' ).forEach(
				function (el) {
					new PillPicker( el );
				}
			);
